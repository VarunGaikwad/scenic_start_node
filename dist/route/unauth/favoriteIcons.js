...
const favoriteIconRouter = (0, express_1.Router)();
...
async function scrapeIconFromSite(domain) {
    ...
    try {
        const { data: html } = await axios_1.default.get(baseUrl, {
            timeout: 5000,
            headers: {
...
favoriteIconRouter.get("/", async (req, res) => {
    ...
    try {
        ...
        const { data: existingFile } = await supabase_1.default.storage
            .from(BUCKET_NAME)
            .download(fileName);
        if (existingFile) {
...
exports.default = favoriteIconRouter;
