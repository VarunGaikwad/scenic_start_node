...
const MODEL_NAME = process.env.GEMINI_MODEL || "gemini-robotics-er-1.5-preview";
...
const LANGUAGES = {
    ...
    hinglish: {
        label: "Hindi",
        extraInstruction: "Write the translation in Latin script (Hinglish). Do NOT use Devanagari. Use natural conversational Hinglish with common English mixing.",
    },
};
...
translationRouter.get("/", async (req, res) => {
    try {
        ...
        const result = (await Promise.race([
            translationPromise,
            new Promise((_, reject) => setTimeout(() => reject(new Error("Translation timeout")), REQUEST_TIMEOUT)),
        ]));
