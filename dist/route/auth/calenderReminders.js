...
const calendarReminderRouter = (0, express_1.Router)();
/**
 * @swagger
 * tags:
 ...
 */
calendarReminderRouter.post("/", async (req, res) => {
    const { type, title, createdAt, userId, completed, date, time, description, priority, location, } = req.body;
 ...
 *         description: Server error
 */
calendarReminderRouter.get("/", async (req, res) => {
 ...
 *         description: Server error
 */
calendarReminderRouter.get("/due/today", async (_req, res) => {
 ...
 *         description: Server error
 */
calendarReminderRouter.get("/user/:userId", async (req, res) => {
 ...
 *         description: Server error
 */
calendarReminderRouter.get("/:id", async (req, res) => {
    const { id } = req.params;
    try {
        const db = await (0, db_1.connectDB)();
        const calendarCollection = db.collection("calendar_reminders");
 ...
 *         description: Server error
 */
calendarReminderRouter.put("/:id", async (req, res) => {
    const { id } = req.params;
    const { type, title, completed, date, time, description, priority, location, } = req.body;
 ...
 *         description: Server error
 */
calendarReminderRouter.patch("/:id", async (req, res) => {
    const { id } = req.params;
    const allowedFields = [
        "type",
        "title",
    ...
    const patchData = { updatedAt: new Date() };
    fields.forEach((key) => {
    ...
    try {
        const db = await (0, db_1.connectDB)();
        const calendarCollection = db.collection("calendar_reminders");
        const result = await calendarCollection.updateOne({ _id: new mongodb_1.ObjectId(id) }, { $set: patchData });
        if (result.matchedCount === 0) {
            return res.status(404).json({ error: "Reminder not found" });
        }
 ...
 *         description: Server error
 */
calendarReminderRouter.delete("/:id", async (req, res) => {
    const { id } = req.params;
    try {
        const db = await (0, db_1.connectDB)();
        const calendarCollection = db.collection("calendar_reminders");
...
exports.default = calendarReminderRouter;
