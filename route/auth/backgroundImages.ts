...
import { admin, AuthRequest } from "../../middleware";
...
const backgroundImagesRouter = Router();
 ...
 *       - bearerAuth: []
 */
backgroundImagesRouter.get("/", async (req: AuthRequest, res: Response) => {
  try {
...
backgroundImagesRouter.delete(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      const doc = await db.findOne({ _id: new ObjectId(id as string) });
      if (!doc) {
...
export default backgroundImagesRouter;
