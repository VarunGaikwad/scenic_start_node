...
if (!process.env.OPENWEATHER_API) {
  console.error(
    "⚠️  WARNING: OPENWEATHER_API environment variable is not set!",
  );
}
...
temperatureRouter.get("/", async (req: Request, res: Response) => {
  try {
    const now = Date.now();
...
async function fetchLatLong(ip: string) {
  try {
...
async function fetchTemperatureByIP(ip: string) {
...
async function fetchTemperatureByCoords(lat: number, lon: number) {
