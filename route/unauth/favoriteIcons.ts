...
const favoriteIconRouter = Router();
...
async function scrapeIconFromSite(domain: string) {
...
favoriteIconRouter.get("/", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const tryDomains: string[] = [domain];
    const parsed = parse(domain);
    if (parsed?.domain && parsed.domain !== domain)
      tryDomains.push(parsed.domain);

    let bestFavicon: any = null;
...
export default favoriteIconRouter;
